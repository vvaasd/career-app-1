...
export const useFiltersCounter = (group='') => {
    ...
    const is_experience_false = radios.includes('experience=experience-false') ? -1 : 0
    ...
    const experienceLength = radios.filter(el => el.includes('experience')).length + is_experience_false
    const part_timeLength = checkboxes.filter(el => el.includes('part_time')).length
    const periodLength = radios.filter(el => el.includes('period')).length + is_period_false
    ...
    const salaryLength = radios.filter(el => el.includes('salary')).length + only_with_salary + is_salary_false

    const defaultFoltersCount = is_salary_false + is_experience_false + is_period_false

    const allFilterLength = areasLength + checkboxesLength + radiosLength + technologiesLength + defaultFoltersCount
